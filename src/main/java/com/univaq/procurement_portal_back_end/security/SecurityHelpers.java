...
public class SecurityHelpers {
    ...
    public static HttpSession checkSession(HttpServletRequest r, boolean loginAgeCheck) {
        ...
        // Added by Batoul
        // Validate the JWT stored in the session
        String jwt = (String) s.getAttribute("session-id");
    ...
    public static HttpSession createSession(HttpServletRequest request, String username, int userid, String userType) {
        ...
        s.setAttribute("session-id", jwt);

    // Optionally, set the JWT as a cookie or response header
    // response.addHeader("Authorization", "Bearer " + jwt);
  //   Cookie jwtCookie = new Cookie("session-id", jwt);
...
//se una sessione è già attiva, rimuoviamola e creiamone una nuova
    ...
    private static String getPasswordHashPBKDF2(String password, byte[] salt) throws NoSuchAlgorithmException, InvalidKeySpecException {
        ...
        String testWord = bytesToHexString(salt) + bytesToHexString(digest);
        return testWord;
    }
    ...
    public static boolean checkPasswordHashPBKDF2(String password, String passwordhash) throws NoSuchAlgorithmException, InvalidKeySpecException {
        byte[] salt = new byte[16];
        System.arraycopy(hexStringToBytes(passwordhash), 0, salt, 0, 16);
        boolean test = (getPasswordHashPBKDF2(password, salt)).equals(passwordhash);
        return test;
    }
